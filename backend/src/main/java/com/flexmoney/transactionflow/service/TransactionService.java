...
import com.flexmoney.transactionflow.core.error.TransactionException;
...
//            );
            ...
            log.error("User is not approved with any lenders");
            throw new TransactionException(ApiResponseCodeEnum.INVALID_TRANSACTION.getCode(), "UnApproved user","You are not approved by any lenders",HttpStatus.BAD_REQUEST);
        }
        ...
        Optional<EUserVerificationInfo> detailsDTO = essentialDetailsRepository.findById(detailsId);
    ...
    public ResponseEntity<TransactionResponse> InitiateTxn(TransactionDTO transactionDTO) {
        ...
        if (check2 == true) {
            ...
            if (!check) {
                ...
                long userId = EUserVerificationInfo.getUserId();
    ...
    public boolean checkTxnCountValues(UUID detailsId, String checkFor){
        ...
        String remark="";
        Integer cnt=0;
        if(EUserVerificationInfo == null) {
            status = false;
            remark = "Invalid Transaction ID";
        } else {
            Integer txnCount = EUserVerificationInfo.getTxnCount();
            Integer PanOTPCount = EUserVerificationInfo.getPanOTPCount();
            Integer MobOTPCount = EUserVerificationInfo.getMobOTPCount();
            ...
            if(cnt >= 3){
                ...
                if(checkFor == "txncount") {
                    ...
                    Date d1 = EUserVerificationInfo.getCreatedAt();
                    Date d2 = new Date();
    ...
    public boolean checkIfTxnIsCompleted(UUID detailsId) {
        ...
        EUserVerificationInfo EUserVerificationInfo = essentialDetailsRepository.findById(detailsId).get();
        String detailsStatus = EUserVerificationInfo.getStatus();
